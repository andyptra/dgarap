...
$mahasiswa=mysql_query("select * from usergarap where username='$_SESSION[namauser]'") or die("gagal".mysql_error());
$dmahasiswa=mysql_fetch_array($mahasiswa);
...
$_SESSION['iduser']=$dmahasiswa[iduser];
...
?>
  ...
  <head>
    ...
    <title>Ujian Online <?php echo"$course"; ?></title>
    <!-- Bootstrap Core CSS -->
  ...
  <body>
    ...
    <div class="container">
        ...
        <!-- Page Heading -->
          ...
          <div class="col-lg-4">
      <b>No Ujian<span class="an">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;:&nbsp;</span> <strong></strong> <?php echo"$dmahasiswa[id_userk]";?></b><br>
            <b>Nama<span class="an">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;:&nbsp;</span> <?php echo"$dmahasiswa[username]";?></b>
            </div>
             <div class="col-lg-5">
      <b>Mata Kuliah<span class="an">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; : </span><?php echo"$dtmatkul[nama_matkul]";?></b> <br>
        ...
        <p><form class="form-horizontal" role="form" id='login' method="post" action="mengerjakan-soal-sudah-selesai.html" >
          ...
          <?php 
          $jml_soal=$dujiant[jsoal];
          if($dujiant[mode_soal]==1){
          $res = mysql_query("select * from soal where id_ujian='$id_ujian' ORDER BY RAND() limit $jml_soal") or die(mysql_error());
          }
          else {
          $res = mysql_query("select * from soal where id_ujian='$id_ujian' ORDER BY id asc limit $jml_soal") or die(mysql_error());
          }
        ...
        <button id='<?php echo $i;?>' class='next btn btn-primary' type='button'>Next</button> 
                ...
                <input type="radio" checked="checked" style='display:none' value="F" id='radio1_<?php echo $result['id'];?>' name='<?php echo $result['id'];?>' />                                                                      
                   ...
                   <?php }elseif($i==$rows){?>
                    ...
                    <font size="+1"><b><?php echo"A";?>.</font></b><input type="radio" value="<?php echo"A";?>" id='radio1_<?php echo $result['id'];?>' name='<?php echo $result['id'];?>' class="radio radio-primary" onClick="document.getElementById('aku<?php echo $i;?>').setAttribute('class', 'klik');"/>
        <?php echo $result['a'];?>
                   <br/>
    ...
    <hr>                
                     <button id='<?php echo $i;?>' class='previous btn btn-primary' type='button'>Previous</button>
                      <button id='<?php echo $i;?>' class='next btn btn-primary' type='submit' onclick="confirmAction(this, 'Are you sure you wish to remove this?', performDelete); return false;">
Selesai</button>                
                    </div>
          <?php } $i++;} ?>
...
<div id="message"><p></p></div>
        ...
        <br/><br/>
          ...
          if($dujiant[mode_soal]==1){
           $res1=mysql_query("select * from soal where id_ujian='$id_ujian' ORDER BY RAND() limit $jml_soal") or die(mysql_error());
          }
          else {
         $res1=mysql_query("select * from soal where id_ujian='$id_ujian' ORDER BY id asc limit $jml_soal") or die(mysql_error());
...
if(isset($_POST[1])){ 
  ...
  // echo $query;exit;
   ...
   $response=mysql_query("select id,jawab from soal where id IN($order) ORDER BY FIELD(id,$order)")   or die(mysql_error());
   $right_answer=0;
   $wrong_answer=0;
   $unanswered=0;
		...
		$('#questiona').removeClass('hide');	 
			 ...
			 var thisName = $(this).attr('name');
   			 var qNum = thisName.substring(1);
			 			 $( "#log"+last ).html(css('color','#2980b9') + "");
...
<script type="text/javascript">
    ...
    {
        return 'Jawaban yang telah anda kerjakan akan hilang, silahkan pilih stay on this page untuk Melanjutkan dan menyelesaikan ujian anda';
    }
