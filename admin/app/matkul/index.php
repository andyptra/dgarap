...
(function($) {
  ...
  $(document).ready(function(e) {
    ...
    var kd_k = 0;
    var main = "app/matkul/k.data.php";
    ...
    $("#data-ptra").load(main); 
		...
		$(document).on("click", ".ubah, .tambah", function () {
			...
			var url = "app/matkul/k.form.php";
			// ambil nilai id dari tombol ubah
			kd_k = this.id;
			
			if(kd_k != 0) {
				// ubah judul modal dialog
				$("#myModalLabel").html("Ubah mata kuliah");
			} else {
		...
		$(document).on("click", ".hapus", function () {
			var url = "app/matkul/k.proses.php";
			// ambil nilai id dari tombol hapus
			kd_k = this.id;
			
			// tampilkan dialog konfirmasi
			var answer = alertify.set({ buttonFocus: "cancel" });
			alertify.confirm("Apakah anda yakin mau menghapus data mata kuliah ini?", function (e) {
				if (e) {
					...
					alertify.success("Data mata kuliah Berhasil di hapus");
				} else {
		...
		$("#simpan-k").bind("click", function(event) {
			...
			var v_nama_matkul 		= $('input:text[name=nama_matkul]').val();
	
		 	// mengirimkan data ke berkas transaksi.proses.php untuk di proses
			
			$.post(url, {nama_matkul: v_nama_matkul, id: kd_k} ,function() {
				// tampilkan data k yang sudah di perbaharui
				// ke dalam <div id="data-ptra"></div>
				$("#data-ptra").load(main);
			...
			var answer = alertify.success("Data mata kuliah Berhasil Di simpan");
